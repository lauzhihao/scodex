...
| 命令 | 作用 |
| --- | --- |
| `scodex` | 刷新额度；如果当前账号 usage 健康、有 5h 则剩余至少 20%、有 weekly 则剩余大于 5%，就继续使用它，否则切换到最佳账号，然后启动或恢复 Codex |
| `scodex launch` | 默认行为的显式写法 |
| `scodex auto` | 刷新额度；当前账号可用则保留，否则切换到最佳账号；没有可用账号时不切换、不自动登录；不启动 Codex |
...
- 刷新后，如果当前账号 usage 健康、有 5h 则剩余至少 20%、有 weekly 则剩余仍然大于 5%，`launch` 会直接继续使用它，不再重新对所有账号打分选号
...
- 会刷新额度；如果当前账号 usage 健康、有 5h 则剩余至少 20%、有 weekly 则剩余大于 5% 就继续使用它，否则切换到最佳账号
- 号池里没有可用账号时，不切换，也不自动发起设备登录
- 不会启动 Codex
...
- 订阅账号可用条件：至少有一个额度窗口；已返回的 `5h` 剩余至少 20%；已返回的 weekly 剩余大于 5%。缺失的窗口不当成耗尽
- 在可用账号之间，先按 `5h` 额度分档（无 5h 窗口的账号用 weekly 分档顶上），再依次比较 weekly 额度分档、weekly 重置时间、credits、额度同步时间和账号更新时间
