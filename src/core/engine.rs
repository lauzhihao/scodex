...
pub fn ensure_best_account<A: AppAdapter>(
    ...
    if let Some(current) = adapter
        .find_current_account(state, live_identity.as_ref())
        .cloned()
    {
        let usage = state.usage_cache.get(&current.id).cloned().unwrap_or_default();
        if adapter.is_account_usable(state, &current, &usage) {
            if perform_switch {
    ...
    if let Some(best) = adapter.select_best_account(state).cloned() {
