...
pub const DEFAULT_BUNDLE_DIR: &str = ".scodex-account-pool";
pub const BUNDLE_FILENAME: &str = "bundle.enc.json";
pub const BUNDLE_KEY_ENV: &str = "SCODEX_POOL_KEY";
...
pub fn push_account_pool(
    ...
    let bundle = build_repo_bundle(state, now_ts)?;
    ...
    git_commit(&git_bin, &checkout.checkout_dir, now_ts)?;
...
pub fn pull_account_pool(
    ...
    let bundle_data = decrypt_bundle_file(&bundle_path, &bundle_key)?;
    let bundle: RepoBundle = serde_json::from_slice(&bundle_data)
...
pub fn resolve_bundle_dir(bundle_dir: Option<&str>) -> Result<PathBuf> {
    let configured =
...
fn build_repo_bundle(state: &State, now_ts: i64) -> Result<RepoBundle> {
...
fn git_commit(git_bin: &Path, checkout_dir: &Path, now_ts: i64) -> Result<()> {
    let message = format!("scodex encrypted account pool sync {now_ts}");
    let output = run_git(
        git_bin,
        ["-C", ""],
        None,
...
fn find_program(candidates: &[&str]) -> Option<PathBuf> {
    candidates
        .iter()
        .find_map(|candidate| crate::core::storage::find_in_path(candidate))
}
...
mod tests {
    ...
    fn build_git_ssh_command_quotes_plain_path() {
        assert_eq!(
            build_git_ssh_command(PathBuf::from("/tmp/id_ed25519").as_path()),
            "ssh -i '/tmp/id_ed25519' -o IdentitiesOnly=yes"
        );
    }
    ...
    fn build_git_ssh_command_handles_spaces() {
        assert_eq!(
            build_git_ssh_command(PathBuf::from("/tmp/My Key").as_path()),
            "ssh -i '/tmp/My Key' -o IdentitiesOnly=yes"
        );
    }
    ...
    fn build_git_ssh_command_escapes_single_quote() {
        assert_eq!(
            build_git_ssh_command(PathBuf::from("/tmp/it's-key").as_path()),
            "ssh -i '/tmp/it'\\''s-key' -o IdentitiesOnly=yes"
        );
    }

    #[test]
    fn overwrite_local_account_pool_replaces_existing_accounts() -> Result<()> {
        let state_dir = std::env::temp_dir().join(format!("scodex-git-test-{}", uuid::Uuid::new_v4()));
        fs::create_dir_all(state_dir.join("accounts/old"))?;
        fs::write(state_dir.join("accounts/old/auth.json"), "{}")?;
        ...
        assert_eq!(state.accounts[0].email, "api@example.com");
        assert!(state_dir.join("accounts/acct-2/auth.json").exists());
        assert!(!state_dir.join("accounts/old").exists());
