...
| Command | Purpose |
| --- | --- |
| `scodex` | Refresh usage, keep the current account when its usage is healthy, 5h remaining is at least 20% (if present), and weekly remaining is above 5% (if present), otherwise switch to the best account, then launch or resume Codex |
| `scodex launch` | Explicit form of the default behavior |
| `scodex auto` | Refresh usage, keep the current account when it is usable, otherwise switch to the best account; if none are usable, do not switch or start login; does not launch Codex |
...
- after refresh, if the current account's usage is healthy, its 5h remaining is at least 20% when that window exists, and its weekly remaining is still above 5% when that window exists, `launch` keeps using it instead of re-scoring all accounts
...
- refreshes usage and keeps the current account when its usage is healthy, 5h remaining is at least 20% (if present), and weekly remaining is above 5% (if present); otherwise it switches to the best account
- if no stored account is usable, it does not switch and does not start device login
...
- a subscription account is usable when at least one quota window exists, returned `5h` remaining is at least 20%, and returned weekly remaining is above 5%; a missing window is not treated as exhausted
- among usable accounts, selection ranks by `5h` quota tier (weekly-only accounts reuse the weekly tier here), then weekly quota tier, weekly reset time, credits, usage sync time, and account update time
